...
public class RecordLock {
	...
	public TreeMap<Integer, LockType> queuedList = null;
	...
	/*
	 * This method records the lock acquisition, but does not first check to see
	 * if the source transaction is allowed to have it.
	 */
	private void acquire(LockType sourceLockType) {
		...
		if (grantedList.containsKey(sourceLockType.lockHolder.tid)) {
			LockType existingLockType = grantedList
					.get(sourceLockType.lockHolder.tid);

			if ((existingLockType.lockType
					.equals(LockType.LockTypeFlag.PROC_READ) || existingLockType.lockType
					.equals(LockType.LockTypeFlag.TRAN_READ))
					&& (sourceLockType.lockType
							.equals(LockType.LockTypeFlag.PROC_WRITE) || sourceLockType.lockType
							.equals(LockType.LockTypeFlag.TRAN_WRITE))) {
			...
			// Store the reference in the grantedRecLockList in the parent
			// Record Lock tree.
			parentRecLockTree.grantedRecLockTypeList.put(
					sourceLockType.lockHolder.tid, sourceLockType);

			// Store a reference to the granted lock in the txn for quick
			// release.
	...
	public Transaction attemptAcquireForNextQueuedTxn() {
		...
		if (nextLockType != null) {
			if (canAcquire(nextLockType)) {
				...
				parentRecLockTree.queuedRecLockTypeList
						.remove(nextLockType.lockHolder.tid);
		...
		if (parentRecLockTree.isBlockedByFileLock(sourceLockType.lockHolder)) {
			...
			if (grantedList.isEmpty()) {
				...
				if (conflictingTxns.isEmpty()) {
					// There are transactions with locks on this record, but
					// they are compatible.
					return true;
				} else {
					...
					while (iter.hasNext()) {
						...
						// Attempt to wound any younger transaction holding a
						// conflicting lock,
						// but still return *false* because it needs to queue up
						// until the younger txns
						// abort.
	 ...
	 * type objects and returns a list of transactions that are in conflict with
	 * the source.
	 */
	private TransactionList getConflictingTxns(LockType sourceLockType) {
		...
		for (Map.Entry<Integer, LockType> entry : grantedList.entrySet()) {
			...
			if (sourceLockType.lockType == LockType.LockTypeFlag.PROC_READ) {
				...
				// Process Write not compatible with Process/Transaction Write
				// or Process Read.
				if ((currLockType.lockType == LockType.LockTypeFlag.PROC_WRITE)
						|| (currLockType.lockType == LockType.LockTypeFlag.TRAN_WRITE)
				...
				// Transaction Write not compatible with Process/Transaction
				// Write or Read.
				conflictingList.add(currLockType.lockHolder);
			}
		}
	...
	public LockType getOldestInQueue() {
		...
		for (Map.Entry<Integer, LockType> entry : queuedList.entrySet()) {
			...
			if (targetLockType != null) {
				if (currLockType.lockHolder.txnStart
						.isBefore(targetLockType.lockHolder.txnStart)) {
